...
Ситуация немного сложнее, если у нас есть вложенный список. Тогда нам понадобится укзать функцию, которая решит, что делать с этим списком.

Специально для этого есть набор фукнции в модуле `Access`, так что необходимость писать свою функцию возникает редко:
