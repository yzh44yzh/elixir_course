defmodule Lesson_03.Task_03_04_AtomTuple do
  ...
  def is_point_inside_figure(point, {:circle, center, radius}) do
    distance(point, center) <= radius
  end

  def is_point_inside_figure({:point, x, y}, {:rect, left_top, right_bottom}) do
...
defmodule Task_04_Test do
  ...
  test "point inside circle" do
    point = {:point, 50, 50}
    assert is_point_inside_figure(point, {:circle, {:point, 10, 10}, 100})
    assert not is_point_inside_figure(point, {:circle, {:point, -10, -10}, 20})
  end

  test "point inside rect" do
    point = {:point, -10, 20}
    assert is_point_inside_figure(point, {:rect, {:point, -20, 30}, {:point, 20, 10}})
    assert not is_point_inside_figure(point, {:rect, {:point, 0, 0}, {:point, 10, 10}})
  end
